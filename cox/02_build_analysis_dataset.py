...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--censor-date",
        help=(
            "Administrative censoring date in YYYY-MM-DD format. "
            "Recorded in the data dictionary for provenance."
        ),
    )
    parser.add_argument(
...
def build_data_dictionary(
    ...
    lines.extend(
        [
            ...
            "## Genetic PCs",
            "",
            "- `genetic_pc1_filled` through `genetic_pc20_filled`: "
            "compatibility column names for genetic PCs; missing values are "
            "preserved for model-specific complete-case fitting",
            "",
...
def main() -> None:
    ...
    merged = cov.merge(prot, on="Participant.ID", how="left").merge(
        prs,
        on="Participant.ID",
        how="left",
    )
    ...
    variables_for_missingness = (
        list(CONTINUOUS_SPECS)
        + ["f_31", "f_20116_0", "f_1558_0"]
        + GENETIC_PC_COLUMNS
    )
